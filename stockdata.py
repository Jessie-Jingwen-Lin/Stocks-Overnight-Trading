#this file downloads data and extracts data for the needed time point, 
...
def get_all_tickers():
    #'nasdaqlisted.txt' has tickers from nasdaq; 'otherlisted.txt' has tickers from other sources e.g. NYSE
    ...
    stocknames1 = stocknames1.loc[stocknames1['Test Issue']=='N',:]['Symbol'] #get rid of Test Issue = Y
    stocknames2 = stocknames2.loc[stocknames2['Test Issue']=='N',:]['ACT Symbol']
    # stocknames1 = stocknames1['Symbol']
    # stocknames2 = stocknames2['ACT Symbol']
    tickers = sorted(list(set(list(stocknames1) + list(stocknames2))))[:50]
...
def extract_overnight_times_60_min(df):
    ninethirty_index = df.index.map(lambda date_time: date_time.hour == 9 and date_time.minute == 30 and date_time.second == 0)
    two_thirty_index = df.index.map(lambda date_time: date_time.hour == 14 and date_time.minute == 30 and date_time.second == 0)

    prices_morning = df.loc[ninethirty_index, :].copy()
    ...
    morning_tuples_index = [('9:30', tup) for tup in prices_morning.columns.values]
