...
def download(tickers, start=None, end=None, actions=False, threads=True,
    ...
    if threads:
        if threads is True:
            ...
            threads = min([len(tickers), 32])
        # _multitasking.set_max_threads(threads)


        download_ticker_lambda = Closure(closure_converted_download_threaded, period=period, interval=interval,
                                   start=start, end=end, prepost=prepost,
                                   actions=actions, auto_adjust=auto_adjust,
                                   back_adjust=back_adjust, proxy=proxy, rounding=rounding)
